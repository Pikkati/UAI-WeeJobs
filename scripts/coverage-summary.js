...
files.sort(
  (a, b) =>
    b.uncovered - a.uncovered || a.percentStatements - b.percentStatements,
);
...
files.slice(0, 30).forEach((f) => {
  ...
  console.log(
    `${String(f.percentStatements).padStart(5)}% | ${String(f.uncovered).padStart(3)}/${String(f.totalStatements).padStart(3)} | ${String(f.coveredFunctions).padStart(3)}/${String(f.totalFunctions).padStart(3)} | ${rel}`,
  );
});
console.log('\nTop app/ files (same ordering):');
const appFiles = files.filter((f) =>
  f.file.includes(path.sep + 'app' + path.sep),
);
...
const totals = files.reduce(
  (acc, f) => {
    acc.total += f.totalStatements;
    acc.covered += f.coveredStatements;
    acc.funcTotal += f.totalFunctions;
    acc.funcCovered += f.coveredFunctions;
    return acc;
  },
  { total: 0, covered: 0, funcTotal: 0, funcCovered: 0 },
);
const totalPct =
  totals.total === 0
    ? 100
    : Math.round((totals.covered / totals.total) * 10000) / 100;
const funcPct =
  totals.funcTotal === 0
    ? 100
    : Math.round((totals.funcCovered / totals.funcTotal) * 10000) / 100;
console.log(
  `\nGlobal statement coverage: ${totalPct}% (${totals.covered}/${totals.total})`,
);
console.log(
  `Global function coverage: ${funcPct}% (${totals.funcCovered}/${totals.funcTotal})`,
);
