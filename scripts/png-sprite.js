...
if (!fs.existsSync(ICONS_DIR)) {
  ...
  console.log(
    'Place PNG icons (same width) in that folder to generate a sprite.',
  );
  process.exit(0);
}
...
try {
  ...
  console.log(
    'The `sharp` package is not installed. To generate PNG sprites automatically, run:',
  );
...
async function buildSprite() {
  const imgs = await Promise.all(
    files.map((f) =>
      sharp(path.join(ICONS_DIR, f))
        .metadata()
        .then((m) => ({ file: f, meta: m })),
    ),
  );
  ...
  for (const img of imgs) {
    compositeInputs.push({
      input: path.join(ICONS_DIR, img.file),
      top: y,
      left: 0,
    });
    y += img.meta.height;
