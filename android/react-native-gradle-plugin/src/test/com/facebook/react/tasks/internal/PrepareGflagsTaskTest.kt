...
"""
  ...
  """
    ...
    fun normalizeLines(s: String) =
      s.replace("\r\n", "\n").lines().map { it.trimStart() }.filter { it.isNotEmpty() }.joinToString("\n")

    assertEquals(normalizeLines(declareFile.readText()), normalizeLines(expectedDeclare))
