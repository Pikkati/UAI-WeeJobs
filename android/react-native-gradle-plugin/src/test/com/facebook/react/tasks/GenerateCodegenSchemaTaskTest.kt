...
class GenerateCodegenSchemaTaskTest {
  ...
  fun setupCommandLine_onWindows_willSetupCorrectly() {
    ...
    val cliPath = try {
      File(codegenDir, "lib/cli/combine/combine-js-to-schema-cli.js").relativeTo(project.projectDir).path
    } catch (e: IllegalArgumentException) {
      File(codegenDir, "lib/cli/combine/combine-js-to-schema-cli.js").absolutePath
    }

    val schemaPath = try {
      File(outputDir, "schema.json").relativeTo(project.projectDir).path
    } catch (e: IllegalArgumentException) {
      File(outputDir, "schema.json").absolutePath
    }

    val jsRootPath = try {
      jsRootDir.relativeTo(project.projectDir).path
    } catch (e: IllegalArgumentException) {
      jsRootDir.absolutePath
    }
