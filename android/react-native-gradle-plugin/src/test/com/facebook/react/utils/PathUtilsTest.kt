...
class PathUtilsTest {
  ...
  fun detectOSAwareHermesCommand_withProvidedCommand() {
    val expected = "./my-home/hermes".replace('/', File.separatorChar)
    val result = detectOSAwareHermesCommand(tempFolder.root, "./my-home/hermes")
    assertThat(result.replace('/', File.separatorChar)).isEqualTo(expected)
  }
  ...
  fun detectOSAwareHermesCommand_withHermescBuiltLocally() {
    ...
    val expected =
      tempFolder.newFile(
        "node_modules/react-native/ReactAndroid/hermes-engine/build/hermes/bin/${getHermesCBin()}"
      )
    val actual = detectOSAwareHermesCommand(tempFolder.root, "")
    val resolved = if (File(actual).isAbsolute) actual else File(tempFolder.root, actual).absolutePath

    assertThat(resolved).isEqualTo(expected.absolutePath)
  }
  ...
  fun detectOSAwareHermesCommand_withProvidedCommand_takesPrecedence() {
    ...
    val result = detectOSAwareHermesCommand(tempFolder.root, "./my-home/hermes")
    val normalized = result.replace('\\', File.separatorChar).replace('/', File.separatorChar)
    assertThat(normalized).isEqualTo("./my-home/hermes".replace('/', File.separatorChar))
  }
  ...
  fun getBuiltHermescFile_withOverride() {
    assertThat(getBuiltHermescFile(tempFolder.root, "/home/ci/hermes"))
      .isEqualTo(File("/home/ci/hermes/build/bin/${getHermesCBin()}"))
  }
