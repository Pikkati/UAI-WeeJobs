...
class BundleHermesCTaskTest {
  ...
  fun getBundleCommand_returnsCorrectCommand() {
    ...
    if (bundleCommand.firstOrNull() == "cmd") {
      assertThat(bundleCommand).containsExactly(*expectedBundleWin.toTypedArray())
    } else {
      assertThat(bundleCommand).containsExactly(*expectedBundleUnix.toTypedArray())
    }
  }
  ...
  fun getHermescCommand_returnsCorrectCommand() {
    ...
    if (hermesCommand.firstOrNull() == "cmd") {
      assertThat(hermesCommand).containsExactly(*expectedHermesWin.toTypedArray())
    } else {
      assertThat(hermesCommand).containsExactly(*expectedHermesUnix.toTypedArray())
    }
  }
  ...
  fun getComposeSourceMapsCommand_returnsCorrectCommand() {
    ...
    val expectedComposeUnix = listOf(
        "node",
        "arg1",
        "arg2",
    ...
    if (composeSourcemapCommand.firstOrNull() == "cmd") {
      assertThat(composeSourcemapCommand).containsExactly(*expectedComposeWin.toTypedArray())
    } else {
      assertThat(composeSourcemapCommand).containsExactly(*expectedComposeUnix.toTypedArray())
    }
  }
