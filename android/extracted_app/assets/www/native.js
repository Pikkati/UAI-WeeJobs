    ...
    console[method] = function (...args) {
      ...
      try {
        if (
          ...
          typeof window.androidBridge.logConsole === 'function'
        ) {
          // Android logging
          window.androidBridge.logConsole(
            JSON.stringify({
              level: method,
...
function parseNativeResponse(nativeResponseString) {
  try {
    const parsedResponse =
      typeof nativeResponseString === 'string'
        ? JSON.parse(nativeResponseString)
        : nativeResponseString;

    if (parsedResponse && parsedResponse.type === 'data') {
      return parsedResponse.data;
    } else if (parsedResponse && parsedResponse.type === 'error') {
      throw new Error(
        parsedResponse.message || 'Native call returned an error',
      );
    } else {
      throw new Error(
        `Invalid response structure from native: ${JSON.stringify(parsedResponse)}`,
      );
    }
  } catch (e) {
    if (
      e.message.includes('Invalid response structure') ||
      e.message.includes('Native call returned an error')
    ) {
      throw e;
    }
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (
        ...
        if (payload) {
          console.log(
            `Sending to Android bridge (${methodName}):`,
        ...
        reject(
          new Error(`Native bridge not available for method: ${methodName}`),
        );
      }
    } catch (e) {
...
window.NativeStripeECE = {
  ...
  getShopPayInitParams: function () {
    try {
      if (
        window.androidBridge &&
        typeof window.androidBridge.getShopPayInitParams === 'function'
      ) {
        const nativeResponseString =
          window.androidBridge.getShopPayInitParams();
        return parseNativeResponse(nativeResponseString);
      } else {
        throw new Error(
          'Native bridge not available for method: getShopPayInitParams',
        );
      }
    } catch (e) {
