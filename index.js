...
(function () {
  try {
    if (typeof globalThis !== 'undefined') {
      globalThis.__EXPO_ROUTER_INSPECTOR_EARLY = {
        ts: Date.now(),
        source: 'index-top',
      };
    }
    if (typeof console !== 'undefined') {
      if (typeof console.error === 'function') {
        console.error(
          '[expo-router-inspector-very-early] bundle-top initialized',
...
try {
  ...
  console.log(
    '[index] __EXPO_ROUTER_KEYS at entry:',
    typeof globalThis !== 'undefined'
      ? globalThis.__EXPO_ROUTER_KEYS && globalThis.__EXPO_ROUTER_KEYS.length
      : 'no-global',
  );
} catch (e) {
  // eslint-disable-next-line no-console
...
try {
  ...
  try {
    ...
    console.log(
      '[index] FileSystemLegacy available:',
      !!FileSystemLegacy,
      'writeAsStringAsync:',
      typeof (FileSystemLegacy && FileSystemLegacy.writeAsStringAsync),
    );
    if (
      FileSystemLegacy &&
      typeof FileSystemLegacy.writeAsStringAsync === 'function'
    ) {
    ...
    console.log(
      '[index] expo-file-system/legacy load failed:',
      ...
      const payload = JSON.stringify({
        keysCount:
          (globalThis.__EXPO_ROUTER_KEYS &&
            globalThis.__EXPO_ROUTER_KEYS.length) ||
          0,
        ts: Date.now(),
      });
      try {
      ...
      const targets = [
        {
          ...
          path: FileSystem.documentDirectory
            ? `${FileSystem.documentDirectory}routes_manifest_seen.txt`
            : null,
        },
        {
          name: 'appCache',
          path: FileSystem.cacheDirectory
            ? `${FileSystem.cacheDirectory}routes_manifest_seen.txt`
            : null,
        },
        {
          name: 'appTemp',
          path: FileSystem.temporaryDirectory
            ? `${FileSystem.temporaryDirectory}routes_manifest_seen.txt`
            : null,
        },
        {
          name: 'sdcardDownload',
          path: '/sdcard/Download/routes_manifest_seen.txt',
        },
      ];
...
try {
  ...
  const ToastAndroid =
    (RN && RN.ToastAndroid) ||
    (RN && RN.default && RN.default.ToastAndroid) ||
    null;
  if (ToastAndroid && typeof ToastAndroid.show === 'function') {
    try {
      const count =
        ...
        'no-global';
      ToastAndroid.show(
        `[index] __EXPO_ROUTER_KEYS=${count}`,
        ToastAndroid.LONG,
      );
    } catch (_) {}
  }
