...
const ITEMS = [
  {id: 'profile', title: 'Complete profile', description: 'Add name, photo, and a short bio.'},
  {id: 'verify', title: 'Verify email', description: 'Confirm your email address so you can receive messages.'},
  {id: 'payment', title: 'Add payment method', description: 'Add a card or payment method for transactions.'},
  {id: 'first-job', title: 'Post your first job', description: 'Create a listing to attract applicants.'}
];
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {ITEMS.map((it) => {
        ...
        return (
          <TouchableOpacity key={it.id} style={styles.row} onPress={() => toggle(it.id)}>
            <View style={[styles.checkbox, done && styles.checkboxDone]} />
            <View style={styles.meta}>
              <Text style={[styles.title, done && styles.titleDone]}>{it.title}</Text>
      ...
      <View style={styles.actions}>
        ...
        <TouchableOpacity style={[styles.actionBtn, styles.reset]} onPress={reset}>
...
const styles = StyleSheet.create({
  ...
  actionBtn: {paddingVertical: 10, paddingHorizontal: 14, backgroundColor: '#0a84ff', borderRadius: 6},
