...
export default function ApproveQuoteScreen() {
      ...
      if (mode === 'estimate') {
        ...
        if (success) {
          Alert.alert(
            ...
            [{ text: 'OK', onPress: () => router.push(`/job/tracking?jobId=${jobId}`) }]
          );
        }
      } else {
  ...
  const sendChangeRequest = () => {
    ...
    if (!trimmedMessage) {
      Alert.alert('Add a message', 'Please explain what changes you would like.');
      return;
    }
  ...
  const labourAmount = mode === 'estimate' 
    ? (job.estimate_hours || 0) * (job.estimate_hourly_rate || 0)
    : (job.quote_labour || 0);
  const materialsAmount = mode === 'estimate' 
    ? (job.estimate_materials || 0)
    : (job.quote_materials || 0);
  const total = mode === 'estimate' 
    ? (job.estimate_total || 0) 
    : (job.quote_total || 0);
  const notes = mode === 'estimate' 
    ? job.estimate_notes 
    : job.quote_notes;
    
  const remainingBalance = Math.max(0, total - depositPaid);

  const getTitle = () => mode === 'estimate' ? 'Review Estimate' : 'Review Quote';
  const getCardTitle = () => mode === 'estimate' ? 'Provisional Estimate' : 'Quote from Tradesperson';
  const getApproveButtonText = () => mode === 'estimate' ? 'Acknowledge Estimate' : 'Approve Quote';
  const getInfoText = () => mode === 'estimate' 
        ...
        {mode === 'quote' && (
          <View style={styles.paymentSummary}>
            ...
            <View style={styles.summaryRow}>
              ...
              <Text style={styles.summaryValue}>£{remainingBalance.toFixed(2)}</Text>
            </View>
          </View>
        )}

        <View style={[styles.infoBox, mode === 'estimate' && styles.infoBoxEstimate]}>
          <Ionicons 
            name={mode === 'estimate' ? 'information-circle' : 'information-circle'} 
            size={20} 
        ...
        {mode === 'estimate' && (
          <View style={styles.estimateNote}>
            ...
            <Text style={styles.estimateNoteText}>
              Final cost will be based on actual time and materials. You&apos;ll review the invoice before paying.
            </Text>
          </View>
        )}
