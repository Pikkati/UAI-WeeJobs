...
async function mockStripePayFinal(
  jobId: string,
  amount: number,
): Promise<{ ok: boolean; id: string }> {
  await wait(800);
...
export function JobsProvider({ children }: { children: ReactNode }) {
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      ...
      if (AsyncStorageLocal && AsyncStorageLocal.setItem) {
        await AsyncStorageLocal.setItem(
          JOBS_CACHE_KEY,
          JSON.stringify(data || []),
        );
      }
      ...
      try {
        ...
        const testCache =
          typeof global !== 'undefined' && (global as any).__TEST_JOBS_CACHE__;
        if (testCache) {
          try {
            const parsed =
              typeof testCache === 'string' ? JSON.parse(testCache) : testCache;
        ...
        debugLog(
          'JOBS_FETCH_ASYNCSTORAGE_TYPE',
          typeof AsyncStorageLocal,
          AsyncStorageLocal,
        );
        debugLog('JOBS_CACHE_KEY', JOBS_CACHE_KEY);
        let cached =
          AsyncStorageLocal && AsyncStorageLocal.getItem
            ? await AsyncStorageLocal.getItem(JOBS_CACHE_KEY)
            : undefined;
        debugLog(
          'JOBS_FETCH_GETITEM_CALLS',
          AsyncStorageLocal &&
            (AsyncStorageLocal.getItem as any).mock &&
            (AsyncStorageLocal.getItem as any).mock.calls,
        );
        ...
        if (
          !cached &&
          AsyncStorageLocal &&
          (AsyncStorageLocal.getItem as any)
        ) {
          debugLog(
            'JOBS_FETCH_HAS_GETMOCKIMPL',
            typeof (AsyncStorageLocal.getItem as any).getMockImplementation,
          );
          if ((AsyncStorageLocal.getItem as any).getMockImplementation) {
            try {
              ...
              if (typeof impl === 'function') {
                ...
                const resolvedAlt =
                  alt && typeof (alt as any).then === 'function'
                    ? await alt
                    : alt;
          ...
          try {
            ...
            if (
              !cached &&
              gm &&
              Array.isArray(gm.results) &&
              gm.results.length > 0
            ) {
              const last = gm.results[gm.results.length - 1].value;
              const resolved =
                last && typeof (last as any).then === 'function'
                  ? await last
                  : last;
  ...
  const fetchInterests = useCallback(
    async (jobId: string): Promise<JobInterest[]> => {
      try {
  ...
  const expressInterest = useCallback(
    async (
      ...
      try {
        ...
        const insertRes: any = await supabase.from('job_interests').insert({
          job_id: jobId,
          tradie_id: userId,
          status: 'interested',
  ...
  const selectTradesman = useCallback(
    async (
      jobId: string,
      tradieId: string,
      pricingType: PricingType,
  ...
  const payDeposit = useCallback(
    async (
      ...
      if (error) {
        throw new Error(
          error.message || 'Failed to create deposit payment intent',
        );
      }
  ...
  const sendEstimate = useCallback(
    async (jobId: string, estimate: Estimate): Promise<boolean> => {
      try {
        await supabase
          .from('jobs')
          .update({
  ...
  const acknowledgeEstimate = useCallback(
    async (jobId: string): Promise<boolean> => {
      try {
        await supabase
          .from('jobs')
  ...
  const sendQuote = useCallback(
    async (jobId: string, quote: Quote): Promise<boolean> => {
      try {
        await supabase
          .from('jobs')
          .update({
  ...
  const sendInvoice = useCallback(
    async (jobId: string, invoice: Invoice): Promise<boolean> => {
      try {
        await supabase
          .from('jobs')
          .update({
  ...
  const confirmCompletion = useCallback(
    async (
      ...
      try {
        const updateField =
          role === 'customer' ? 'customer_confirmed' : 'tradie_confirmed';
    ...
    async (
      ...
      try {
        ...
        const depositRefunded =
          role === 'tradie' ? true : currentJob?.status === 'booked';
        const newStatus =
          role === 'customer' ? 'cancelled_by_customer' : 'cancelled_by_tradie';
      ...
      if (role === 'customer') {
        switch (jobStatus) {
          ...
          case 'awaiting_customer_choice':
            return [
              {
                label: 'Choose Tradesperson',
                action: 'choose_tradesperson',
                variant: 'primary',
              },
            ];
          case 'booked':
            return [
              {
                label: isHourly ? 'View Estimate' : 'Track Job',
                action: 'track_job',
                variant: 'primary',
              },
          ...
          case 'awaiting_final_payment':
            return [
              { label: 'Pay Now', action: 'pay_final', variant: 'primary' },
            ];
          case 'paid':
            return [
              {
