...
export function parseServerError(err: any): ParsedError {
  ...
  if (typeof err === 'object' && typeof err.status === 'number') {
    ...
    if (status === 429) {
      const ra = err.headers && (err.headers.get ? err.headers.get('retry-after') : err.headers['retry-after']);
      const retryAfterSeconds = ra ? parseInt(String(ra), 10) || null : null;
      return { message: 'Too many requests. Please try again later.', isRateLimited: true, retryAfterSeconds };
    }
  ...
  if (err && typeof err === 'object' && 'message' in err) {
    ...
    if (lower.includes('too many') || lower.includes('rate limit') || lower.includes('rate-limited')) {
      return { message: 'Too many attempts. Please try again later.', isRateLimited: true, retryAfterSeconds: null };
    }
    return { message };
  }
