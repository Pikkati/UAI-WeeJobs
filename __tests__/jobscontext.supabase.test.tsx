...
(global as any).__TEST_USE_AUTH__ = () => ({
  user: { id: 'u1', subscription_plan: 'pro' },
});
// Ensure AuthContext hook resolves to a test user (module-level mocks)
jest.mock(
  '../context/AuthContext',
  () => ({ useAuth: () => ({ user: { id: 'u1', subscription_plan: 'pro' } }) }),
  { virtual: true },
);
...
function TestInvoker() {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      const se = await sendEstimate('job1', {
        hours: 2,
        hourlyRate: 20,
        materials: 10,
        total: 50,
      ...
      const si = await sendInvoice('job1', {
        hours: 1,
        hourlyRate: 20,
        materials: 5,
        total: 25,
