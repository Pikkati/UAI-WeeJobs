...
describe('CustomerProfile module', () => {
  ...
  test('module loads as a function', () => {
    ...
    jest.doMock('../lib/supabase', () => ({ supabase: { from: () => ({ select: () => ({ eq: () => ({ then: (fn: any) => fn({ data: [] }) }) }) }) } }));
    jest.doMock('../context/AuthContext', () => ({ useAuth: () => ({ user: { id: 'u1', name: 'Test User', email: 'a@b.com' }, logout: jest.fn() }) }));
