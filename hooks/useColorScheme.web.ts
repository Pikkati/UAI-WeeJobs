...
export function useColorScheme() {
  ...
  useEffect(() => {
    ...
    const forceHydrate = typeof global !== 'undefined' && (global as any).__TEST_FORCE_HYDRATE;
    if (isJest && !forceHydrate) return undefined;

    let cancelled = false;
    const rafId = typeof requestAnimationFrame === 'function'
      ? requestAnimationFrame(() => { if (!cancelled) setHasHydrated(true); })
      : undefined;

    // Also schedule a microtask so tests that await a microtask observe hydration.
    Promise.resolve().then(() => { if (!cancelled) setHasHydrated(true); });
